...
additional=TRUE, use_qualV=FALSE){
  n = length(modelled)
  ...
  if(sum(is.na(modelled)) |sum(is.na(measured)) ){
  ...
  if(sum(!is.na(modelled)) <10 | (sd(measured)==0 & sd(modelled)==0)  ){
       t_test <- NA
  } else {
  ...
  if(additional){
      ...
      max_reldiff <- max(diff_measured/diff_modelled, na.rm=TRUE)
  ...
  if(additional&&use_qualV){
      ...
      IRMSE=IRMSE, I=I, lagtime=lagtim, rel_diff=max_reldiff, DE=DE,
